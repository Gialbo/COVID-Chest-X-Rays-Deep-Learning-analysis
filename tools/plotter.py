...
def plot_accuracies(history, isMCD=False, selected_run='3'):
  if isMCD:
    ...
    train_acc = history[selected_run].history["accuracy"]
...
def plot_recalls(history, isMCD=False, selected_run='3'):
  with plt.style.context('fivethirtyeight'):
    ...
    if isMCD:
      ...
      train_rec_0 = history[selected_run].history["recall_6"]
      valid_rec_0 = history[selected_run].history["val_recall_6"]
      train_rec_1 = history[selected_run].history["recall_7"]
      valid_rec_1 = history[selected_run].history["val_recall_7"]
      train_rec_2 = history[selected_run].history["recall_8"]
      valid_rec_2 = history[selected_run].history["val_recall_8"]
...
def plot_precisions(history, isMCD=False, selected_run='3'):
  with plt.style.context('fivethirtyeight'):
    ...
    if isMCD:
      ...
      train_pre_0 = history[selected_run].history["precision_6"]
      valid_pre_0 = history[selected_run].history["val_precision_6"]
      train_pre_1 = history[selected_run].history["precision_7"]
      valid_pre_1 = history[selected_run].history["val_precision_7"]
      train_pre_2 = history[selected_run].history["precision_8"]
      valid_pre_2 = history[selected_run].history["val_precision_8"]
